...
export default function SimulationLab({ agentOutputs, setActivePage }) {
  ...
  if (!a3?.scenarios) {
    return (
      <div className="max-w-5xl mx-auto">
        <h2 className="text-lg font-bold text-[#f9fafb] mb-6">SIMULATION LAB</h2>
        <EmptyState
          icon="🎮"
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        <h2 className="text-lg font-bold text-[#f9fafb]">SIMULATION LAB</h2>
        <p className="text-xs text-[#6b7280] mt-1">
          Simulation Confidence: <span className="text-[#9ca3af] font-mono">{Math.round((a3.simulation_confidence || 0) * 100)}%</span> ·
    ...
    >
      ...
      <div className={isRecommended ? 'mt-6' : ''}>
        <div className="flex items-center gap-2 mb-3">
          <span className="text-xl font-black text-[#4b5563]">#{sc.id}</span>
          <PriorityBadge level={sc.type} />
        </div>
        <h4 className="text-base font-bold text-[#f9fafb] mb-3">{sc.title}</h4>
        <p className="text-xs text-[#9ca3af] mb-4 leading-relaxed">{sc.description}</p>
        ...
        <div className="space-y-3 text-xs border-t border-[#1f2937] pt-4">
          <div className="flex justify-between">
            <span className="text-[#6b7280]">Timeline</span>
            <span className="text-[#9ca3af] font-mono font-semibold">{sc.timeline_days} days</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-[#9ca3af] text-right max-w-[60%] leading-snug">{sc.resources_required}</span>
          </div>
        </div>
        ...
        {/* Projected outcome */}
        <div className="mt-4 p-3 bg-[#0a0f1e] rounded-lg border border-[#1f293750]">
          <p className="text-[10px] text-[#6b7280] uppercase tracking-wide mb-1">Projected Outcome</p>
          <p className="text-xs text-[#d1d5db] leading-relaxed">{sc.projected_outcome}</p>
        </div>
      </div>
    </div>
  );
