  ...
  if (!currentBrief || !brief) {
    return (
      <div className="max-w-5xl mx-auto">
        <h2 className="text-lg font-bold text-[#f9fafb] mb-6">COMMANDER BRIEF</h2>
        <EmptyState
          icon="📋"
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-0">
      ...
      <div className="no-print flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-bold text-[#f9fafb]">COMMANDER BRIEF</h2>
          <p className="text-xs text-[#6b7280] mt-0.5">Output — Final Intelligence Product</p>
        </div>
        <div className="flex gap-2">
          <button onClick={handlePrint} className="px-4 py-2 rounded-lg bg-[#1f2937] text-[#9ca3af] text-xs font-semibold hover:bg-[#374151] transition-colors border border-[#374151]">
            🖨️ Print Brief
          </button>
      ...
      <div className="bg-[#111827] border border-[#1f2937] border-t-0 rounded-b-xl p-6 md:p-8 space-y-8">
        {/* Brief info */}
        <div className="flex flex-col sm:flex-row justify-between gap-2 pb-4 border-b border-[#1f2937]">
          <div>
            <p className="text-xs text-[#6b7280]">Brief ID</p>
            <p className="text-sm font-mono text-[#f9fafb] font-bold">{brief.brief_id}</p>
          </div>
          <div>
          ...
          <div className="sm:text-right">
            ...
            <p className="text-sm font-mono text-[#9ca3af]">{new Date(brief.generated_at || currentBrief.generatedAt).toLocaleString()}</p>
          </div>
        </div>
        ...
        <Section number="1" title="EXECUTIVE SUMMARY">
          <div className="flex flex-wrap items-center gap-3 mb-3">
            ...
            <span className="text-sm text-[#9ca3af]">Threat Score: <span className="text-[#f9fafb] font-bold">{brief.threat_score}/10</span></span>
          ...
          <p className="text-sm text-[#d1d5db] leading-relaxed">{brief.executive_summary}</p>
        </Section>
        ...
        <Section number="2" title="THREAT ASSESSMENT">
          <div className="space-y-3">
            ...
            {a2?.sentiment_analysis && (
              <div className="mt-3 p-4 bg-[#0a0f1e] rounded-lg border border-[#1f293750]">
                <p className="text-xs text-[#6b7280] uppercase tracking-wider mb-2">Sentiment Analysis</p>
                <div className="flex flex-wrap gap-4 text-xs">
                  <span className="text-[#9ca3af]">Score: <span className="text-[#f9fafb] font-mono font-bold">{a2.sentiment_analysis.sentiment_score}</span></span>
                  <span className="text-[#9ca3af]">Trend: <PriorityBadge level={a2.sentiment_analysis.trending_direction} /></span>
                  {a2.sentiment_analysis.breakdown && (
                    <>
                      <span className="text-[#ef4444]">Negative: {a2.sentiment_analysis.breakdown.negative}%</span>
                      <span className="text-[#9ca3af]">Neutral: {a2.sentiment_analysis.breakdown.neutral}%</span>
            ...
            {a2?.reasoning && (
              <div className="mt-4 p-4 bg-[#0a0f1e] rounded-lg border border-[#1f293750]">
                <p className="text-xs text-[#6b7280] uppercase tracking-wider mb-2">Detailed Analysis</p>
                <p className="text-xs text-[#9ca3af] leading-relaxed whitespace-pre-line">{a2.reasoning}</p>
              </div>
            )}
          </div>
        </Section>
            ...
            <ol className="space-y-4">
              {brief.key_findings.map((f, i) => (
                <li key={i} className="flex gap-3">
                  <span className="text-sm font-mono text-[#4b5563] shrink-0 w-6">{i + 1}.</span>
                  <div className="flex-1">
                    <p className="text-sm text-[#d1d5db] leading-relaxed">{f.finding}</p>
          ...
          <Section number="4" title="STRATEGIC SCENARIOS & PROJECTED OUTCOMES">
            <div className="space-y-4">
              {a3.scenarios.map((sc, i) => (
                <div key={i} className="p-4 bg-[#0a0f1e] rounded-lg border border-[#1f293750]">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="font-mono text-[#4b5563] font-bold">#{sc.id}</span>
                    <PriorityBadge level={sc.type} />
                    <span className="text-sm font-bold text-[#f9fafb]">{sc.title}</span>
                    {sc.id === a3.recommended_scenario_id && (
                      <span className="badge bg-[#10b98120] text-[#10b981] border border-[#10b98140]">★ Recommended</span>
                    )}
                  </div>
                  <p className="text-xs text-[#9ca3af] leading-relaxed mb-2">{sc.description}</p>
                  <div className="flex flex-wrap gap-4 text-xs text-[#6b7280]">
                    <span>Success: <span className="text-[#10b981] font-mono">{Math.round(sc.success_probability * 100)}%</span></span>
                    <span>Timeline: <span className="text-[#9ca3af] font-mono">{sc.timeline_days}d</span></span>
                    <span>Risk: <PriorityBadge level={sc.risk_level} /></span>
                  </div>
                  {/* Scenario civilian impact */}
                  {sc.civilian_impact && (
                    <div className="mt-2 pt-2 border-t border-[#1f293730] flex flex-wrap gap-4 text-xs text-[#6b7280]">
            ...
            {brief.scenarios_summary && (
              <p className="text-sm text-[#d1d5db] leading-relaxed mt-4">{brief.scenarios_summary}</p>
            )}
          </Section>
        )}
        ...
        {civilianImpact && (
          <Section number="5" title="CIVILIAN IMPACT">
            ...
            {brief.civilian_impact_summary && (
              <p className="text-sm text-[#d1d5db] leading-relaxed">{brief.civilian_impact_summary}</p>
            )}
          </Section>
        )}
            ...
            <ol className="space-y-4">
              {brief.top_3_recommendations.map((rec, i) => (
                <li key={i} className="flex gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-[#f9fafb] mb-1">{rec.action}</p>
                    <p className="text-xs text-[#9ca3af] leading-relaxed mb-2">{rec.rationale}</p>
                    <div className="flex flex-wrap gap-2">
                      <PriorityBadge level={rec.urgency} />
                      <PriorityBadge level={rec.risk} />
                    </div>
                    <p className="text-[10px] text-[#4b5563] mt-1.5">📎 Source: {rec.source}</p>
                  </div>
                </li>
              ))}
            </ol>
        ...
        {brief.sources_cited && (
          <Section number="7" title="SOURCES & CITATIONS">
            <ul className="space-y-1">
              {brief.sources_cited.map((s, i) => (
                <li key={i} className="flex items-center gap-2 text-xs text-[#9ca3af]">
                  <span className="text-[#4b5563] font-mono w-5">{i + 1}.</span>
                  {s}
                </li>
              ))}
            </ul>
            ...
            {a4?.citation_chain && (
              <div className="mt-4 pt-3 border-t border-[#1f293730]">
                <p className="text-[10px] text-[#4b5563] uppercase tracking-wider mb-2">Traceable Citation Chain</p>
                <div className="space-y-1.5">
                  {a4.citation_chain.map((c, i) => (
                    <div key={i} className="flex items-start gap-2 text-[11px]">
                      <span className="text-[#3b82f6]">↳</span>
                      <span className="text-[#9ca3af]">{c.claim}</span>
                      <span className="text-[#4b5563]">— {c.agent}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {/* 8. NEXT ASSESSMENT */}
        <div className="pt-6 border-t border-[#1f2937] flex justify-between text-sm">
          <span className="text-[#6b7280]">Next Assessment:</span>
          <span className="text-[#f9fafb] font-bold">{brief.next_assessment}</span>
        </div>
      </div>
...
function Section({ number, title, children }) {
  return (
    <div>
      <h3 className="flex items-center gap-2 text-sm font-bold text-[#f9fafb] mb-4 uppercase tracking-wider">
...
function Metric({ label, value, color = '#f9fafb' }) {
  return (
    <div className="text-center p-3 bg-[#0a0f1e] rounded-lg border border-[#1f293750]">
